...
const MenuItem: React.FC<_props> = ({ type, url, title, content,  openTab,
  ...
  if (content !== null && typeof content === 'string') {
    return <Custom title={title} url={_titleURL} content={content}  openTab={openTab} setOpenTab={setOpenTab} />;
  }

  if (content !== null && 'dataType' in content) {
    if (type === 'BRANDS' && content.dataType === 'BRANDS') {
      return <Brand title={title} url={paths.BRAND} content={content.brands}   openTab={openTab} setOpenTab={setOpenTab} />;
    }

    if (type === 'CATEGORY' && content.dataType === 'CATEGORIES') {
      return (
        <Category title={title} url={_titleURL} content={content.categories}   openTab={openTab}  setOpenTab={setOpenTab} />
      );
    }
  }
  if (type === 'TOPIC') {
