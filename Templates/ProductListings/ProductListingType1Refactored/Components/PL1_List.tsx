...
import PL1_Product from './PL1_Product';
...
interface _Props {
  ...
  brandOrCategory: __pageTypeConstant.brand | __pageTypeConstant.category;
}

const PL1_List: React.FC<_Props> = ({ brandOrCategory, pageId, list }) => {
  ...
  const updateQuery = (pageNo: number) => {
    ...
    if (router.query?.sort) {
      query = `?sort=${router.query.sort}&page=${pageNo}`;
    }
